...
/*
static ErrorState_t LCD_WriteHex_Digit(uint8 num)
{
	ErrorState_t ret= E_OK;
	switch(num)
	{
...
ErrorState_t LCD_WriteHex(uint8 num)
{
	ErrorState_t ret= E_OK;
	//uint8 result=0;
	ret|= LCD_WriteHex_Digit(num/16);
	ret|= LCD_WriteHex_Digit(num%16);
	return ret;
}

ErrorState_t LCD_WriteHex2(u32 num)
{
	ErrorState_t ret= E_OK;
	uint8 i=0, rem, str[8];
	s8 j;
	...
	for(j=i; j>0; j--)
	{
		ret|= LCD_WriteHex_Digit(str[j-1]);
	}
	return ret;
}
*/
